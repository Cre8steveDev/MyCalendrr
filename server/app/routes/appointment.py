    ...
    try:
        ...
        return (
            jsonify({
                ...
                False,
                "message":
                f"Error creating appointment. Please try again later.",
            }),
            400,
        )
...
def update_public_appointment(appointment_id: str):
    ...
    if not appointment or appointment.user_id != user.id:
        return (
            jsonify({
                ...
                False,
                "message":
                "Oops! Appointment not found or you don't have permission to update it.",
            }),
            404,
        )
    ...
    try:
        ...
        return (
            jsonify({
                "success": False,
                "message": f"Error updating appointment."
            }),
            400,
        )
...
def get_all_appointments():
    ...
    try:
        ...
        appointments_dict = [
            appointment.to_dict_for_table() for appointment in all_appointments
        ]
        ...
        return (
            jsonify({
                ...
                False,
                "message":
                f"Error retrieving appointments. Please try again later.",
            }),
            400,
        )
...
def get_appointment(appointment_id: str):
    try:
        ...
        if not appointment:
            return (
                jsonify({
                    "success": False,
                    "message":
                    f"Invalid Appointment ID or the appointment has been deleted.",
                    "appointment": None,
                }),
                404,
            )
        ...
        return (
            jsonify({
                "success": False,
                "message": f"An Error occured. Please try again later.",
                "appointment": None,
            }),
            400,
        )
...
def book_appointment():
    ...
    if not appointment_id or not transaction_reference or not amount_paid:
        return jsonify({
            "success":
            False,
            "message":
            "Invalid Booking Data. Please validate your entry.",
        }), 403
    ...
    try:
        ...
        if not appointment:
            return jsonify({
                ...
                False,
                "message":
                "Oops! Something went wrong. Unable to validate appointment.",
            }), 404
        ...
        if not book:
            return jsonify({
                ...
                False,
                "message":
                "Sorry. The Date has already been booked.",
            }), 403

        appointment.user.bookings.append(data)
        topup_success = appointment.user.topup_account(float(amount_paid))

        if not topup_success:
            # Notify Admins with transaction reference
            pass
